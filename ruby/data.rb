namespace :data do
	...
	task :pushassets do
		begin
			...
			# ... but the webserver user doesn't have the write permission to the ".." directory.
	...
	def normalise_assets(params)
		if exists?(:webserver_user)
			...
			run "chmod 0775 #{shared_path}/assets", params
			...
			run "find #{shared_path}/assets -type d -user `whoami` -exec chmod 2775 {} \\;", params
